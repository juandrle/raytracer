...
class Triangle:
    def __init__(self, v0, v1, v2, diffuse, mirror=10):
        self.v0 = v0
        self.v1 = v1
        self.v2 = v2
        self.diffuse = diffuse
    ...
    def normal(self):
        ...
        return self.edge1.cross(self.edge2).norm()
    
    def intersect(self, O, D):
        tvec = O - self.v0
        self.edge1 = self.v1 - self.v0
        self.edge2 = self.v2 - self.v0
        pvec, qvec, svec = (vec3(tvec.cross(self.edge1).dot(self.edge2), D.cross(self.edge2).dot(tvec),tvec.cross(self.edge1).dot(D)).__mul__(1/D.cross(self.edge2).dot(self.edge1))).components()
        cond = np.where((qvec >= 0) & (qvec <= 1) & (svec >=0) & ((qvec+svec) <= 1) & (pvec >= 0))
        res = np.full_like(pvec, FARAWAY)
        res[cond] = pvec[cond]
        return res
...
scene = [
    Sphere(vec3(.4, .6, 0.7), 0.3, rgb(1, 0, 0)),
    Sphere(vec3(-0, 1.2, 0.7),0.3, rgb(0, 0, 1)),
    Sphere(vec3(-.4, .6, 0.7), 0.3, rgb(0, 1, 0)),
    Triangle(vec3(0.4, 0.6, 0.7), vec3(0, 1.2, 0.7), vec3(-0.4, 0.6, 0.7), rgb(1, 1, 0)),
    CheckeredSphere(vec3(0,-99999.5, 0), 99999, rgb(.9, .9, .9), 0.25)
    ]
